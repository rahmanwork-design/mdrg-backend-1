...
router.post('/register', async (req, res) => {
  try {
    ...
    // Set debt amount (default 0 if not provided)
    const debtAmount = total_debt ? parseFloat(total_debt) : 0;

    // Insert new client
    await dbRun(
      `INSERT INTO clients (client_id, email, password, first_name, last_name, company_name, phone, address, city, postcode, total_debt, balance_due)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [clientId, email, hashedPassword, first_name, last_name, company_name, phone, address, city, postcode, debtAmount, debtAmount]
    );
