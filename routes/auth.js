...
const { dbGet, dbRun, generateReferenceNumber } = require('../src/database');
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Generate unique client ID and reference number
    const clientId = 'MDRG' + Date.now().toString(36).toUpperCase();
    const referenceNumber = await generateReferenceNumber();

    // Insert new client
    await dbRun(
      `INSERT INTO clients (client_id, email, password, first_name, last_name, company_name, phone, address, city, postcode, reference_number, original_creditor, account_reference, total_debt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [clientId, email, hashedPassword, first_name, last_name, company_name, phone, address, city, postcode, referenceNumber, original_creditor || null, account_reference || null, total_debt || 0]
    );
