...
// Helper function to generate unique 8-digit reference number
const generateReferenceNumber = async () => {
  const generateRandom8Digit = () => {
    return Math.floor(10000000 + Math.random() * 90000000).toString();
  };
  
  let reference = generateRandom8Digit();
  let isUnique = false;
  let attempts = 0;
  ...
  while (!isUnique && attempts < maxAttempts) {
    const existing = await dbGet(
      'SELECT reference_number FROM cases WHERE reference_number = ?',
      [reference]
    );
    ...
    if (!existing) {
      ...
      reference = generateRandom8Digit();
      attempts++;
    }
  }
...
router.get('/check-reference/:reference', async (req, res) => {
  try {
...
router.post('/:clientId/cases', authenticateToken, async (req, res) => {
  try {
    ...
    // Generate case ID and reference number
    ...
    await dbRun(
      `INSERT INTO cases (case_id, client_id, debtor_name, debtor_company, debtor_email, 
                          ...
                          description, priority, reference_number, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [caseId, clientId, debtor_name, debtor_company, debtor_email, debtor_phone,
       debtor_address, amount_owed, currency || 'GBP', debt_type, description, priority || 'medium', referenceNumber, '']
    );
