...
async function ensureAdminUser() {
  try {
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@mdrg.co.uk';
    const adminPassword = process.env.ADMIN_PASSWORD || 'Admin123!';

    const existingAdmin = await dbGet('SELECT * FROM clients WHERE email = ?', [adminEmail]);

    if (!existingAdmin) {
      const hashedPassword = await bcrypt.hash(adminPassword, 10);
      const clientId = 'ADMIN' + Date.now().toString(36).toUpperCase();

      await dbRun(
        `INSERT INTO clients (client_id, email, password, first_name, last_name, status)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [clientId, adminEmail, hashedPassword, 'Admin', 'User', 'active']
      );
...
const startServer = async () => {
  try {
    await initDatabase();
    await ensureAdminUser(); // Create admin on startup
